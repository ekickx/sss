...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int ismax, wasfloating, isfixed, iscentered, isfloating, isurgent, neverfocus, oldstate, isfullscreen;
	Client *next;
	Client *snext;
	Monitor *mon;
	Window win;
...
manage(Window w, XWindowAttributes *wa)
{
  ...
  if (round_active == 1) drawroundedcorners(c);
	if (c->mon == selmon)
		unfocus(selmon->sel, 0);
	c->mon->sel = c;
	arrange(c->mon);
